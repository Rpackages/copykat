...
#' @param  cell.line if the data are from pure cell line,put TRUE; if cell line data are a mixture of tumor and normal cells, still put FALSE.
...
copykat <- function(rawmat=rawdata, id.type="S", cell.line=F, ngene.chr=5,LOW.DR=0.05, UP.DR=0.1, win.size=25, norm.cell.names="", KS.cut=0.1, sam.name="", distance="euclidean", output.seg=FALSE, plot.genes=TRUE, genome="hg20", n.cores=1,plot.out=T){
